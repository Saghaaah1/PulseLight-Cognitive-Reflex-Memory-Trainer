...
// Caractères personnalisés (cercles vides et pleins)
byte boutonVide[] = {
   0b00000,
   0b00100,
   0b01010,
 ...
 int positions[4][2] = {
   {5, 0}, {8, 0},  // Haut gauche/droite
   {5, 1}, {8, 1}   // Bas gauche/droite
 };
...
void displayInstruction(int boutonIndex) {
  ecran.clear();
  if(boutonIndex == 8 || boutonIndex == 10 ||boutonIndex == 4 ||boutonIndex == 6){
    for(int i = 0; i < 4; i++) {
    ecran.setCursor(positions[i][0], positions[i][1]);
    ecran.write(i == 0 ? byte(1) : byte(0));
  }
  }
  if(boutonIndex == 15 || boutonIndex == 0 ||boutonIndex == 13 ||boutonIndex == 2){
    for(int i = 0; i < 4; i++) {
    ecran.setCursor(positions[i][0], positions[i][1]);
    ecran.write(i == 1 ? byte(1) : byte(0));
  }
  }
  if(boutonIndex == 9 || boutonIndex == 11 ||boutonIndex == 5 ||boutonIndex == 7){
    for(int i = 0; i < 4; i++) {
    ecran.setCursor(positions[i][0], positions[i][1]);
    ecran.write(i == 2 ? byte(1) : byte(0));
  }
  }
  if(boutonIndex == 14 || boutonIndex == 12 ||boutonIndex == 3 ||boutonIndex == 1){
    for(int i = 0; i < 4; i++) {
    ecran.setCursor(positions[i][0], positions[i][1]);
    ecran.write(i == 3 ? byte(1) : byte(0));
    }
  }
...
void displaySerial(int bouton, int zone, int compteur){ // affichage score
   Serial.println("\nEssai n°: "+String(compteur));
   Serial.println("bouton n."+bouton);
   switch (zone) {
...
boolean correctAnswer(int bouton){ //permet de vérifier que le bouton appuyé  correspond à la zone demandée
   int answer = whichAnswer();
   if(answer == bouton){return true;}
   return false;
}
...
void loop(){ //boucle du jeu principal, où rassembler les fonctions afin d’obtenir le mode 1 
   ...
   while (compteur < number_of_turn) {
      ...
      int bouton = chooseButton();
      int zone = zoneOfButton(bouton);
      uint32_t couleur = codeColor(zone);
      displayColor(couleur);s
      displayInstruction(bouton);
      //displaySerial(bouton, zone, compteur);
      int time_before = millis();
      delay(100);
      while(pressed == false){
         if (correctAnswer(bouton)==true){pressed = true;}
      }
