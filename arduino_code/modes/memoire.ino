...
String data_screen = "";
String data_putty = "PUTTY,SEQUENCE,ESSAI,TEMPS,REUSSITE,";
String data_bpm = "BPM,";
int number_of_turn = 3;
...
uint32_t codeColor(int zone){
   switch (zone) {
       ...
       case 4 : return strip.Color(25,25,25); //greyish
...
//display Sequence and endScreen
void displaySequenceScreenSerial() {
   ecran.clear();
   ecran.setCursor(0, 0);
   ecran.print(" 3  ");
   ecran.print("  2  ");
   ...
   displayColor(codeColor(4));
   delay(750);
   int order = 0;
   for (int i = 0; i < tailleSequence; i++) {
       int nextOne = sequence[i];
       ecran.clear();
       displayEndColor();
       switch (nextOne) {
           case 0: ecran.setCursor(0, 0); ecran.print("n : " + String(i+1)); ecran.setCursor(0, 1);ecran.print(" ROUGE ");Serial.println("ROUGE"); displayColor(codeColor(ROUGE)); delay(1000);break;
           case 1: ecran.setCursor(0, 0); ecran.print("n : " + String(i+1)); ecran.setCursor(0, 1);ecran.print(" VERT ");Serial.println("VERT"); displayColor(codeColor(VERT));delay(1000);break;
           case 2: ecran.setCursor(0, 0); ecran.print("n : " + String(i+1)); ecran.setCursor(0, 1);ecran.print(" BLEU ");Serial.println("BLEU"); displayColor(codeColor(BLEU));delay(1000);break;
           case 3: ecran.setCursor(0, 0); ecran.print("n : " + String(i+1)); ecran.setCursor(0, 1);ecran.print(" JAUNE ");Serial.println("JAUNE"); displayColor(codeColor(JAUNE));delay(1000);break;
           default : ;//Serial.print("problème"); delay(1000);
       }
   }
   ecran.clear();
...
void displayEndScreen(int score, int compteur){ // affichage score
   ecran.clear();
   ecran.setCursor(0, 0);
   ecran.print("Score: ");
   ecran.print(score);
...
int correctAnswer(int zone, int answer){ //permet de vérifier que le bouton appuyé  correspond à la zone demandée
   ...
   switch (zone) {
      case 0: if(answer == zone){return 1;}else{return 2;}
      case 1:  if(answer == zone){return 1;}else{return 2;}
      case 2:  if(answer == zone){return 1;}else{return 2;}
      case 3:  if(answer == zone){return 1;}else{return 2;}
      default: return 3;
  }
}
boolean touchedButton(){//return true if a button (any buttons) is touched
  if(whichAnswer()==-1){return false;}
  return true;
}
...
void displayTime(String time) { //affiche si la réponse est bonne ou pas
   ...
   ecran.print("temps :");
   ecran.print(time);
   //Serial.println("temps n°"+String(compteurExterne)+" = "+time+"\n");
   delay(1000);
}
void newData(String time, boolean succes){ //enregistre les nouvelles données temporelles associés aux succès dans un tableau
   String successToString = "";
   if (succes == true){successToString = " REUSSITE";}
   else{successToString = " ECHEC";}
   data_screen = data_screen + String(compteurInterne)+":  "+time + successToString+ "\n" ;
   data_putty = data_putty + String(compteurInterne)+","+time+","+ successToString +",";
}
void displayTimeScreen(String time){ 
   ecran.clear();
...
void loop(){ //boucle du jeu principal, où rassembler les fonctions afin d’obtenir le mode 1 
   while (compteurExterne < number_of_turn) {
      compteurExterne+=1;
      displayEndColor();
      data_screen = data_screen +"SEQUENCE n." + String(compteurExterne)+"\n";
      data_putty = data_putty + String(compteurExterne) +",";
      ...
      while (sequenceRunning == true){
         ...
         generateSequence();
         int n=1;
         displaySequenceScreenSerial();
         displayEndColor();
         for (int i=0; i<tailleSequence; i+=1){
            int nextOne = sequence[i];
            ...
            while(pressed == false){
               ...
               if(answer != -1){
                  if (correctAnswer(nextOne, answer)==1){
                     succes = true;
                     points ++;
                     playSuccessSound();}
                  ecran.clear();
                  ...
                  SEC = 0;
                  MILLI = 0;
                  newData(time, succes);n++;}
               
            }
            compteurInterne ++;
          }
          if (compteurInterne == tailleSequence){
            compteurInterne = 0;
            sequenceRunning = false;
            //Serial.println("Fin de la séquence");
            ecran.clear();
            ecran.print("FIN SEQUENCE n." + String(compteurExterne));
            delay(1500);
            ecran.clear();
            ecran.print("TOUCHER UN BOUTON");
            //Serial.print(String(data_screen));
            while(touchedButton()==false){
            }
            tailleSequence = tailleSequence + 2;
         }
      }
   }
   //if(compteurExterne==number_of_turn){playStopSound();ecran.clear();ecran.print("Fin du jeu");Serial.println("\n****\n"+data_screen+"****");compteurExterne+=1;}
   if(compteurExterne==number_of_turn){playStopSound();ecran.clear();ecran.print("Fin du jeu");Serial.println(data_putty);Serial.println(data_bpm);compteurExterne+=1;}
}
